...
class Task extends React.Component {
    ...
    prettyStatus = () => {
        ...
        if (val === 'Ongoing') {
            return (
                <div className="btn btn-primary btn-block btn-sm" >В процессе</div>
            )
        } else if (val === "Pending") {
            return (
                (daysDiff <= 1) ? <div className="btn btn-danger btn-block btn-sm" >Не завершено</div>
                : <div className="btn btn-warning btn-block btn-sm" >В работе</div>

            )
        } else if (val === "Completed") {
            return (
                <div className="btn btn-success btn-block btn-sm" >Завершено</div>
            )
        } else if (val === "Overdue") {
            return (
                <div className="btn btn-secondary btn-block btn-sm" >Просрочено</div>
            )
        }
    }
